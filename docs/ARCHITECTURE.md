...
| Multi-Signature Verification | Dual-key threshold: `fin-001` (Financier) + `ops-001` (Operations Admin), both required, both must be distinct signers. Pending co-signature state stored in a backend buffer (`pendingMultiSigStore`), not frontend state, governed by the same 5-min `expiresAt` window as the rest of the pipeline. Abandoned partial commands purged and logged on expiry. Dual-Key Mode defaults ON at app load; toggle to Single-Key Mode remains available for sandbox attack demos. Multi-sig extends Check 1 only — does not reorder or replace Checks 2–5. | (decided Session 6) Enterprise governance requires multiple authorization keys for high-risk operations. Backend buffer (not frontend state) supports cross-role/cross-tab co-signing. Same expiry window avoids introducing a second timeout policy. Extending Check 1 keeps the fixed verification order intact. |
